...
Dates in R are stored in UNIX format, meaning that the current day is actually an integer that counts the number of days since January 1, 1970. Which brings us to this: you can *convert* different object types in R, as long as that conversion makes logical sense. For example:
```{r echo=TRUE, eval=TRUE}
as.numeric(Sys.Date())
```
...
This means we can do arithmetic with months. Below you can see that January plus March equals April.
```{r echo=TRUE, eval=TRUE}
months[1] + months[3]
```
